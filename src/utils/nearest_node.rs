...
pub fn add_nearest_node_to_geo_df<'a>(geo_df: DataFrame, nodes_to_match: &DataFrame, target_crs: &str) -> DataFrame {
    ...
    let (id, dist): (Vec<u64>, Vec<f64>) = geo_df.column("lat").unwrap().f64().unwrap().into_iter().zip(geo_df.column("long").unwrap().f64().unwrap().into_iter()).map(|(lat, long)| {
    ...
    return geo_df.lazy().with_columns([Series::new("nearest_node_osm_id".into(), id).lit(), Series::new("nearest_node_distance".into(), dist).lit()]).collect().unwrap();
}
